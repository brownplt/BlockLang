 ...
 */
Blockly.Whalesong.generate_primitive_function_call = function (primitive /*, arguments to function */) { 
    var failure = 'function(v) { throw { message: "Failure!!!", value: v }; }';
    var success = 'function(v) { _temp = v; return v; }';

    if(!primitive) { 
	throw { message: "Too few arguments provided, at least 1 needed" }

    } else {
	var declare_temp = 'var _temp';
	var return_temp = 'return _temp';
	var name = 'plt.baselib.primitives.Primitives[' + Blockly.Whalesong.quote_(primitive) + ']';
	var jsFunc = 'plt.baselib.functions.asJavaScriptFunction(' + name + ')';

	if(arguments.length == 1) { 
	    return jsFunc;

	} else { 
	    var call = jsFunc + '(' + success + ', ' + failure;
	    for(var i = 1; i < arguments.length; i++) { 
		call += ', ' + arguments[i];
	    }
	    call += ')';

	    var function_body = declare_temp + ';\n' + call + ';\n' + return_temp + ';\n';
...
Blockly.Whalesong.math_arithmetic = function() {
  ...
  var code = Blockly.Whalesong.generate_primitive_function_call(primitive_name, argument0, argument1);
