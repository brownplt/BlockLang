...
Blockly.Whalesong.math_arithmetic = function() {
  ...
  var tuple = Blockly.Whalesong.math_arithmetic.OPERATORS[mode];
  var operator = tuple[0];
  var order = tuple[1];
  var argument0 = Blockly.Whalesong.valueToCode(this, 'A', order) || '0';
  var argument1 = Blockly.Whalesong.valueToCode(this, 'B', order) || '0';
  var code;
  // Power in Whalesong requires a special case since it has no operator.
  if (!operator) {
...
Blockly.Whalesong.math_arithmetic.OPERATORS = {
  ADD: [' + ', Blockly.Whalesong.ORDER_ADDITION],
  MINUS: [' - ', Blockly.Whalesong.ORDER_SUBTRACTION],
  MULTIPLY: [' * ', Blockly.Whalesong.ORDER_MULTIPLICATION],
  DIVIDE: [' / ', Blockly.Whalesong.ORDER_DIVISION],
  POWER: [null, Blockly.Whalesong.ORDER_COMMA]  // Handle power separately.
};
...
Blockly.Whalesong.math_single = function() {
  ...
  return [code, Blockly.Whalesong.ORDER_DIVISION];
};
...
Blockly.Whalesong.math_random_int = function() {
  ...
  var argument1 = Blockly.Whalesong.valueToCode(this, 'TO',
      Blockly.Whalesong.ORDER_COMMA) || '0';
  if (!Blockly.Whalesong.definitions_['math_random_int']) {
    var functionName = Blockly.Whalesong.variableDB_.getDistinctName(
        'math_random_int', Blockly.Generator.NAME_TYPE);
    Blockly.Whalesong.math_random_int.random_function = functionName;
    var func = [];
    ...
    func.push('}');
    Blockly.Whalesong.definitions_['math_random_int'] = func.join('\n');
  }
  var code = Blockly.Whalesong.math_random_int.random_function +
      '(' + argument0 + ', ' + argument1 + ')';
  return [code, Blockly.Whalesong.ORDER_FUNCTION_CALL];
};
...
Blockly.Whalesong.math_random_float = function() {
  ...
  return ['Math.random()', Blockly.Whalesong.ORDER_FUNCTION_CALL];
};
