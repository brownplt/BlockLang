...
- [ ] Connect example blocks to testing functionality
...
When you change a function and that causes type error, make the block
that doesn't fit turn red or something, and then also have the
...
Have failed tests change color to red upon failure, and successful
ones to turn green upon success, and then have a tooltip that
explains what went wrong that is active over the whole area of the
test block. 
...
be. Basically the problem is setting everything up to communicate correctly.
