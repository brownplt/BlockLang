...
Order of priorities:
1. Move defined functions to the shared set of blocks. 
2. Separate out argument blocks. 
3. Hook up defined function blocks to the definition, so we can
   change things as needed. 
4. Hook up definition expressions to functions so that we have the
   right semantics. 
5. Add a run button, and show output somehow!
