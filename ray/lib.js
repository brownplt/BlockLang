define(["jquery", "../underscore", "ray"], function($, _, R) {
  ...
  lib.make_predicate = function(r,type) {
    return r.prim(r.spec({'p_args':['x']}), function(x) {
      return r.bool(r.type(x) === type);
    });
  };
  ...
  lib.initialize = function(r) {
    ...
    r.bind("+", r.prim(r.rest_spec('ls'), function(ls) {
	    var sum = _.reduce(ls, function(a, b) { return r.numbers.add(a, b.get()); }, 0);
      return r.num(sum);
	  }));
    ...
    r.bind("list?", r.fn(r.p_spec(['x']),
      r.or(r.app(r.name('null?'), r.args([r.name('x')])),
           r.and(r.app(r.name('pair?'), r.args([r.name('x')])),
                 r.app(r.name('list?'), r.args([r.app(r.name('cdr'), r.args([r.name('x')]))]))
                )
          )));
